...
$mail=$_POST['mail'];
$civil=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['civil']);
$nom=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['nom']);
$prenom=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['prenom']);
$adresse=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['adresse']);
$cp=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['cp']);
$ville=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['ville']);
$pays=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['pays']);
$nl=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['nl']);
$tel=preg_replace("#[^a-zA-Z0-9 יטךכÊֻאגהֲִמן־ֿûשüÛÜפצרװײח]#",' ',$_POST['tel']);
...
mysqli_query($connectionbd,"INSERT INTO client (con,pass,civil,nom,prenom,adresse,code_postal,ville,pays,mail,tel) VALUE ('$con','$pass','$civil','$nom','$prenom','$adresse','$cp','$ville','$pays','$mail','$tel')") OR die(header("location:inscription.php?rep=Cette+adresse+mail+est+deja+utilisי+merci+d'en+utiliser+une+autre"));
setcookie("con",$con,"0","/");
$message="Vous venez de vous inscrire en tant que client sur le site sur le site la rose de l'orient.
Nous nous rיjouissons de vous compter parmi notre aimable clientטle.
Rappelle de vos identifiant:
Login: $mail
Mot de passe: $mdp

En vous remerciant de l'intיrךt que vous portez א la rose de l'orient.
Hakim AZIZI
";
